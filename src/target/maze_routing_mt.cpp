...
namespace maze_routing
{
    ...
    int solve(const char *case_name)
    {
        ...
        vector<pos> my_q[MAX_VAL + 1];
        
        for(int dis = 0, q_id = 0; dis <= limit ; dis++, q_id = q_id==MAX_VAL ? 0 : q_id+1)
        {
            if(q[q_id].empty()) n_empty_q++;
            else n_empty_q=0;
            if(n_empty_q == MAX_VAL) break;
            
            #pragma omp parallel private(my_q)
            {
                #pragma omp for
                for(auto p : q[q_id])
                {
                    ...
                    for(int way = 0; way < (int)(sizeof(dr)/sizeof(dr[0])); way ++)
                    {
                        ...
                        if(Map(subp).val > MAX_VAL) continue; // -1
                    
                        if(Map(subp).out_q == 1 && Map(subp).set != myset) // found
                        #pragma omp critical
                        {
                            char new_dis = Map(p).dis + Map(subp).dis;
                            if(!found) {
                                base = -new_dis;
                            ...
                            else if((char)(new_dis + base) < 0){// wrap to 1 byte
                                base = -new_dis;
                                if(myset == 0) SMID = p, DMID = subp;
                                else SMID = subp, DMID = p;
                            }
                        ...
                        block b_r = Map(subp);
                        if(b_r.in_q) continue;
                        // in_q means (in_q, out_q) = (1, 0) or (1, 1)
                        // if it "in queue" earlier in another set, then it "out queue" earlier too.
                        block b_w = b_r;
                        b_w.in_q = 1;
                        b_w.from = way;
                        b_w.set = myset;
                        b_w.dis = Map(p).dis + b_w.val;// may overflow, that's OK
                        bool success = __sync_bool_compare_and_swap((block_t*)&Map(subp), *(block_t*)&b_r, *(block_t*)&b_w);
                        if(!success) continue;// updated by other threads
                    
                        vector<pos> &nxt_q = my_q[nxt_q_id[q_id + b_r.val]];
                        nxt_q.push_back(subp);
                    }
                }
                #pragma omp critical
                for(int i = 0; i < MAX_VAL + 1; i++) if(i != q_id)
                     q[i].insert(q[i].end(), my_q[i].begin(), my_q[i].end());
